...
def run_omniglot():
    ...
    dst_path = 'result/top3/'
    if os.path.exists(dst_path) is False: os.makedirs(dst_path)
    f = open(dst_path + 'top3_train_info.txt', 'w')
    ...
    # f.write(enc)
    # f.write(dec)
    f.write('** train data =' + str(len(train_dataset)) + '\n')
    f.write('** test data =' + str(len(test_dataset)) + '\n')
    ...
    if args.encoder_checkpoint:
        ...
        f.write('loading encoder checkpoint from {}'.format(args.encoder_checkpoint))
        checkpoint = torch.load(args.encoder_checkpoint)
        enc.load_state_dict(checkpoint['encoder_state'])
    else:
        print('training encoder from scratch!!')
        f.write('training encoder from scratch!!\n')
    ...
    # training
    f.write('episode_idx, np.mean(acc), np.mean(nll), max_batches, used_memory, used_time\n')
    for episode_idx in range(args.n_episodes):
        ...
        f.write("{} acc:{:.3f} nll:{:.3f} b:{} mem:{} [{:.1f} s/it]\n".format(
            episode_idx, np.mean(acc), np.mean(nll), max_batches, used_memory, used_time))
        scheduler.step()
        ...
        if episode_idx % args.test_frequency == 0 and episode_idx != 0:
            ...
            f.write("***  Test -> acc:{:.3f} nll:{:.3f} mem:{} [{:.1f} s/it]\n".format(
                np.mean(acc), np.mean(nll), used_memory, used_time))
